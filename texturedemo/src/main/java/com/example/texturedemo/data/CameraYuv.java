...
public class CameraYuv {
    ...
    public CameraYuv(Context context, int width, int height) {
        mWidth = width;
        mHeight = height;
        String vertexSource = GlslReader.readSourceFromFile(context, R.raw.camera_yuv_vertex);
        int vertexShader = ProgramHelper.getShaderFromSource(GLES20.GL_VERTEX_SHADER, vertexSource);
        String fragmentSource = GlslReader.readSourceFromFile(context, R.raw.camera_yuv_fragment);
        ...
        if (program != -1) {
            ...
            mVTextureHandle = GLES20.glGetUniformLocation(program, "SamplerV");
        }
        ...
        createTextureId(mWidth >> 1, mHeight >> 1, GLES20.GL_LUMINANCE, mUTextureID);
        createTextureId(mWidth >> 1, mHeight >> 1, GLES20.GL_LUMINANCE, mVTextureID);
